...
fun TTTScreen() {
    ...
    val moves = remember {
        mutableStateListOf(true, null,  null, null,  null,  null, null, null, false)
    }
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        val gradientColors= listOf(Color.Black, Color.Green, Color.Red, Color.Blue)
        ...
        if (win.value != null) {
            when (win.value) {
                Win.Player -> {
                    Text(text = "player has won ", fontSize = 26.sp)
                }

                Win.Ai -> {
                    Text(text = "AI has won ", fontSize = 26.sp)
                }

                Win.Draw -> {

                    Text(text = "it's a draw ", fontSize = 26.sp)
                }
...
fun Header(playerTurn:Boolean){
    Row (verticalAlignment = Alignment.CenterVertically,
        ...
        val airBoxColor = if (playerTurn) Color.LightGray else Color.Red
...
fun Board(moves:List<Boolean?>,onTap:(Offset)->Unit) {
    ...
    )
    {
        ...
        Row(
            horizontalArrangement = Arrangement.SpaceEvenly, modifier = Modifier.fillMaxSize(1f)
        ) {
            Column(
                modifier = Modifier
                    .width(2.dp)
...
fun GetComposableFromMove(move:Boolean?){
    when(move) {
        true -> Image(painter = painterResource(id = R.drawable.baseline_close_24)
            , contentDescription =null, modifier = Modifier.fillMaxSize(1f)
            , colorFilter = ColorFilter.tint(Color.Blue)
        )
        false-> Image(painter = painterResource(id = R.drawable.baseline_panorama_fish_eye_24),
            contentDescription = null,
            modifier = Modifier.fillMaxSize(1f)
            , colorFilter = ColorFilter.tint(Color.Red))


        null -> Image(painter = painterResource(id = R.drawable.baseline_10k_24),
            contentDescription =null,
            modifier = Modifier.fillMaxSize(1f))
    }
}